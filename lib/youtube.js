...
export async function downloadVideo(id, repo, callback = () => {}) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      for await (const line of spawn('yt-dlp', `${id} ${cookiesOption} -o ./data/videos/${id}.mp4 --concurrent-fragments 5 -f best[ext=mp4] --write-subs --write-auto-subs --sub-lang en --sub-format vtt --convert-subs srt -k`.split(/ +/))) {
        console.log(line)
        callback(line)
      }
