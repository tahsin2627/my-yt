  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (!fs.existsSync(transcriptPath)) {
        for await (const line of spawn('yt-dlp', `${id} ${cookiesOption} -o ./data/videos/${id} --skip-download --write-subs --write-auto-subs --sub-lang en --sub-format ttml --convert-subs srt`.split(' '))) {
          console.log(line)
          callback(line)
        }
      }
