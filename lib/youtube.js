...
export async function downloadVideo(id, repo, callback = () => {}) {
  return new Promise(async (resolve, reject) => {
    try {
      for await (const line of spawn('yt-dlp', `"${id}" --default-search ytsearch -f best[ext=mp4] -o ./data/videos/${id}.mp4 --write-subs --write-auto-subs --sub-lang en --sub-format ttml --convert-subs srt`.split(' '))) {
        callback(line)
      }
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (!fs.existsSync(transcriptPath)) {
        for await (const line of spawn('yt-dlp', `"${id}" --default-search ytsearch -o ./data/videos/${id} --skip-download --write-subs --write-auto-subs --sub-lang en --sub-format ttml --convert-subs srt`.split(' '))) {
          callback(line)
        }
      }
