...
export async function downloadVideo(id, repo, callback = () => {}) {
  return new Promise(async (resolve, reject) => {
    try {
      for await (const line of execa`yt-dlp --default-search ytsearch -f best[ext=mp4] "${id}" -o ./data/videos/${id}.mp4`.iterable()) {
        callback(line)
      }
...
export async function getVideoSubtitles (id, callback = () => {}) {
  return new Promise(async (resolve, reject) => {
    try {
      for await (const line of execa`yt-dlp --default-search ytsearch --skip-download --write-subs --write-auto-subs --sub-lang en --sub-format ttml --convert-subs srt "${id}" -o ./data/videos/${id}-sub`.iterable()) {
        callback(line)
      }
...
export async function getVideosFor(channelName) {
  ...
  function toInternalVideo(v) {
    ...
    return {
      ...
      thumbnail: `https://img.youtube.com/vi/${v.richItemRenderer.content.videoRenderer.videoId}/mqdefault.jpg`,
