...
export async function downloadVideo(id, repo, callback = () => {}) {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      const video = repo.updateVideo(id, {downloaded: true, location, format})

      if (process.env.SKIP_TRANSCODING !== 'true' && video.location && video.format) {
        ...
        callback(`successfully transcoded video ${video.location}`)
      }
