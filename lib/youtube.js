...
export async function downloadVideo(id, repo, callback = () => {}) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      for await (const line of spawn('yt-dlp', `${cookiesOption} -o ./data/videos/${id}.mp4 ${videoYTDLPArgs} ${subtitlesYTDLPArgs} -- ${id}`.split(/ +/))) {
        console.log(line)
        callback(line)
      }
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (!fs.existsSync(transcriptPath)) {
        for await (const line of spawn('yt-dlp', `${cookiesOption} -o ./data/videos/${id} --skip-download ${subtitlesYTDLPArgs} -- ${id}`.split(/ +/))) {
          console.log(line)
          callback(line)
        }
      }
