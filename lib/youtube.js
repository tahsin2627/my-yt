  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (!fs.existsSync(transcriptPath)) {
        for await (const line of execa`yt-dlp --default-search ytsearch "${id}" -o ./data/videos/${id} --skip-download --write-subs --write-auto-subs --sub-lang en --sub-format ttml --convert-subs srt`.iterable()) {
          callback(line)
        }
      }
