...
export async function downloadVideo(id, repo, callback = () => {}) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      for await (const line of spawn('yt-dlp', `"${id}" ${cookiesOption} --default-search ytsearch --concurrent-fragments 5 -f best[ext=mp4] -o ./data/videos/${id}.mp4 --write-subs --write-auto-subs --sub-lang en --sub-format ttml --convert-subs srt`.split(' '))) {
        console.log(line)
        callback(line)
      }
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (!fs.existsSync(transcriptPath)) {
        for await (const line of spawn('yt-dlp', `"${id}" ${cookiesOption} --default-search ytsearch -o ./data/videos/${id} --skip-download --write-subs --write-auto-subs --sub-lang en --sub-format ttml --convert-subs srt`.split(' '))) {
          console.log(line)
          callback(line)
        }
      }
...
export async function getVideosFor(channelName) {
  try {
    ...
    return videoContents
      .map(toInternalVideo)
      .filter(Boolean)
      .filter(v => v.publishedAt > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000))
      .filter((_,i) => i < 10)
  } catch (error) {
...
export async function getVideoInformation(id) {
  if (id.startsWith('https://')) id = id.replace(/https:\/\/www\.youtube\.com\/watch\?v=/, '')
  try {
    ...
    return {
      channelName: json.playerOverlays.playerOverlayRenderer.autoplay.playerOverlayAutoplayRenderer.byline.runs[0].navigationEndpoint.browseEndpoint.canonicalBaseUrl.replace('/@',''),
      title: json.playerOverlays.playerOverlayRenderer.autoplay.playerOverlayAutoplayRenderer.videoTitle.simpleText,
      url: `https://www.youtube.com/watch?v=${id}`,
      thumbnail: `https://img.youtube.com/vi/${id}/mqdefault.jpg`,
      description: ' ',
      id,
      publishedTime: json.contents.twoColumnWatchNextResults.results.results.contents[0].videoPrimaryInfoRenderer.relativeDateText.simpleText,
      publishedAt: parseRelativeTime(json.contents.twoColumnWatchNextResults.results.results.contents[0].videoPrimaryInfoRenderer.relativeDateText.simpleText),
      viewCount: ' ', // data.viewCountText?.simpleText,
      duration: ' ', // data.lengthText?.simpleText,
    }
  } catch (error) {
