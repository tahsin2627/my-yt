...
export async function downloadVideo(id, repo, callback = () => {}) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (!repo.getVideo(id)) {
        const video = await getVideoInformation(id)
        console.log('downloaded video information', video)
        repo.upsertVideos([video])
        repo.setVideoDownloaded(id)
        console.log('Download completed, new video added', id)
        resolve(video)
        return
      }
...
export async function getVideoInformation(id) {
  ...
  try {
    ...
    return {
      ...
      id,
      publishedTime: '',
      publishedAt: parseRelativeTime('') || new Date(),
      viewCount: '',
      duration: '',
    }
  } catch (error) {
