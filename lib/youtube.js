...
const subtitlesYTDLPArgs = `--write-subs --write-auto-subs --sub-lang en --sub-format vtt --convert-subs srt -k`
const videoYTDLPArgs = `--concurrent-fragments 5 -f best[ext=mp4]`
...
export async function downloadVideo(id, repo, callback = () => {}) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      for await (const line of spawn('yt-dlp', `${id} ${cookiesOption} -o ./data/videos/${id}.mp4 ${videoYTDLPArgs} ${subtitlesYTDLPArgs}`.split(/ +/))) {
        console.log(line)
        callback(line)
      }
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (!fs.existsSync(transcriptPath)) {
        for await (const line of spawn('yt-dlp', `${id} ${cookiesOption} -o ./data/videos/${id} --skip-download ${subtitlesYTDLPArgs}`.split(/ +/))) {
          console.log(line)
          callback(line)
        }
      }
