    ...
    try {
      ...
      if (process.env.SKIP_TRANSCODING !== 'true') {
        try {
          ...
          exec(`ffmpeg -i ${location}  -c:v libx264 -preset veryfast -crf 28 -x264-params opencl=true -vf "format=yuv420p" -profile:v main -movflags +faststart -c:a copy ${location.replace(format, `-h264.${format}`)}`, (error) => {
            if (!error) {
