...
export default class Repository {
  ...
  filterByExcludedTerms (v, excludedTerms = this.getExcludedTerms()) {
    return !excludedTerms.some(term => v.title?.toLowerCase().includes(term) || v.description?.toLowerCase().includes(term))
  }  
  getVideos (query = {}) {
    const isFiltering = Object.keys(query).length > 0 && query.filter
    let videos = this.videos
    .filter(v => query.excluded === 'true' ? true : this.filterByExcludedTerms(v))
    .filter(v => query.downloaded === 'true' ? v.downloaded : true)
