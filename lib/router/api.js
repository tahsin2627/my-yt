...
export default function apiHandler(req, res, repo, connections = [], state = {}) {
  ...
  if (url.pathname === '/api/channels' && req.method === 'DELETE')
    return deleteChannelHandler(req, res, repo, connections)
  if (url.pathname === '/api/download-video' && req.method === 'POST')
    return downloadVideoHandler(req, res, repo, connections, state)
  if (url.pathname === '/api/summarize-video' && req.method === 'POST')
    return summarizeVideoHandler(req, res, repo, connections, state, llmSettings)
  ...
  if (url.pathname === '/api/videos' && req.method === 'GET')
    return searchVideosHandler(req, res, repo, connections)
  ...
  if (url.pathname === '/api/transcode-videos' && req.method === 'GET')
    return getTranscodeVideosHandler(req, res, repo, connections)
  if (url.pathname === '/api/transcode-videos' && req.method === 'POST')
    return setTranscodeVideosHandler(req, res, repo, connections)
  if (url.pathname === '/api/excluded-terms' && req.method === 'GET')
    return getExcludedTermsHandler(req, res, repo, connections)
  if (url.pathname === '/api/excluded-terms' && req.method === 'POST')
    return addExcludedTermHandler(req, res, repo, connections)
  if (url.pathname === '/api/excluded-terms' && req.method === 'DELETE')
    return removeExcludedTermHandler(req, res, repo, connections)
  if (url.pathname.match(/\/api\/videos\/.*/) && req.method === 'GET')
    return watchVideoHandler(req, res, repo)
  if (url.pathname.match(/\/api\/captions\/.*/) && req.method === 'GET')
    return captionsHandler(req, res, repo)
...
async function getTranscodeVideosHandler(req, res, repo) {
  let transcodeVideos = repo.getTranscodeVideosSetting()
  res.writeHead(200, { 'Content-Type': 'application/json' })
