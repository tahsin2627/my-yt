...
function diskUsageHandler(req, res, repo) {
  ...
  const filterFn = onlyIgnored ? video => video.downloaded && video.ignored : video => video.downloaded
...
async function getTranscodeVideosHandler(req, res, repo) {
  let transcodeVideos = repo.getTranscodeVideos()
  res.writeHead(200, { 'Content-Type': 'application/json' })
...
async function setTranscodeVideosHandler(req, res, repo) {
  ...
  repo.setTranscodeVideos(transcodeVideos)
  res.writeHead(200)
  res.end()
}
