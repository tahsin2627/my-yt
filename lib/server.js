...
export function createServer ({repo, connections = []}) {
  ...
  return http.createServer(async (req, res) => {
    ...
    if (url.pathname === '/ignore-video' && req.method === 'POST') 
      return ignoreVideoHandler(req, res, connections)
  ...
  async function downloadVideoHandler(req, res, connections = []) {
    ...
    if (url) {
      ...
      .then((video) => {
        broadcastSSE(JSON.stringify({type: 'new-videos', videos: [video]}), connections)
        broadcastSSE(JSON.stringify({type: 'downloaded', videoId: video.id}), connections)
      })
      .catch((error) => 
  ...
  async function ignoreVideoHandler(req, res) {
    const body = await getBody(req)
