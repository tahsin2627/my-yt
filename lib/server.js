...
export function createServer ({repo, port = 3000, connections = []}) {
  ...
  return http.createServer(async (req, res) => {
    ...
    if (parsedUrl.pathname === '/normalize.css') 
  ...
  function summarizeVideoHandler(req, res, connections = []) {
    let body = ''
    req.on('data', chunk => {
      body += chunk.toString()
    })
    req.on('end', () => {
      ...
      summarizeVideo(id, repo, (line) => 
        broadcastSSE(JSON.stringify({type: 'download-log-line', line}), connections))
      .then((summary) => 
        broadcastSSE(JSON.stringify({type: 'summary', summary, videoId: id}), connections))
      .catch((error) => console.error(error))
