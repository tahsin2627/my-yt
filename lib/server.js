...
export function createServer ({repo, connections = [], updateVideos = true} = {}) {
  if (updateVideos) {
...
function watchVideoHandler(req, res, repo) {
  const id = req.url.replace('/videos/', '').replace(/\.(webm|mp4)$/, '')
  const video = repo.getVideo(id)
...
function diskUsageHandler(req, res, repo) {
  ...
  .reduce((total, video) => {
    try {
      ...
      total += files.reduce((acc, file) => {
        const filesize = fs.statSync(file).size / Math.pow(10, 9)
        return acc + filesize
      }, 0)
      repo.setVideoDownloaded(video.id, false)
      return total
    } catch (err) {
      console.error(err)
      return total
