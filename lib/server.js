...
import { downloadVideo, extractIdFromUrl, isYouTubeUrl } from './youtube.js'
...
export function createServer ({repo, connections = []}) {
  ...
  return http.createServer(async (req, res) => {
    ...
    if (url.pathname === '/api/ignored-videos-disk-space' && req.method === 'GET') 
      return currentDiskSpaceHandler(req, res, repo, connections)
  ...
  function currentDiskSpaceHandler(req, res) {
    const videos = repo.getVideos()
  ...
  function reclaimDiskSpaceHandler(req, res) {
    const videos = repo.getVideos()
    const ignoredVideos = videos.filter(video => video.downloaded)
    ignoredVideos.forEach((video) => {
      try {
        const files = fs.globSync(`./data/videos/${video.id}*`)
        files.forEach(file => fs.unlinkSync(file))
        repo.upsertVideos([Object.assign(video, {downloaded: false, ignored: false})])
      } catch (err) {
